...
namespace Netfluid.Collections
{
	...
	/// </summary>
	public class StringCache<K> : IDisposable
	{
        ...
        public Func<K,string> Load { get; set; }
        public event Action<K,string> OnRemove;
	   ...
	   /// exists in the cache. (Default = false).</param>
		public void AddOrUpdate(K key, string cacheObject, int cacheTimeout, bool restartTimerIfExists = false)
		{
			if (disposed) return;
			...
			try
			{
                ...
                var b = Encoding.UTF8.GetBytes(cacheObject);
        ...
        public string this[K key] => Get(key);
		...
		public string Get(K key)
		{
			...
			try
			{
                ...
                if (cache.TryGetValue(key, out rv))
                    val = Encoding.UTF8.GetString(rv);
                else if (Load != null)
                    val = Load(key);
            }
		...
		public bool TryGet(K key, out string value)
		{
			if (disposed)
			{
				value = null;
			...
			try
			{
				...
				if (cache.TryGetValue(key,out bytes))
				{
                    ...
                    if(OnRemove!=null)
                    {
                        ...
                        {
                            if (OnRemove != null) OnRemove(key,Encoding.UTF8.GetString(bytes));
                        });
                    }
				}
			}
