...
namespace Netfluid.Collections
{
	...
	/// </summary>
	public class ObjectCache<K,T> : IDisposable where T:new()
	{
		#region Constructor and class members
		/// <summary>
		/// Initializes a new instance of the <see cref="Cache{K}"/> class.
		/// </summary>
		public ObjectCache() { }
        ...
        public Func<K,T> Load { get; set; }
        public event Action<K, T> OnRemove;
			...
			try
			{
				...
				if (cache.TryGetValue(key,out bytes))
				{
                    ...
                    if(OnRemove!=null)
                    {
                        ...
                        {
                            if (OnRemove != null) OnRemove(key,BSON.Deserialize<T>(bytes));
                        });
                    }
				}
			}
